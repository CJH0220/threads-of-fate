...
| 文件 | 用途 | 状态 |
|---|---|---|
| `事件配置表.csv` | 事件池（55行中文），策划主表 | P0 权威 |
| `events_complete.csv` | 事件池完整展开版（195行英文），**导出自主表，勿手动编辑** | 导出 |
| `event_outcomes_complete.csv` | 事件结果映射表（114行英文），**导出自主表，勿手动编辑** | 导出 |
| `event_templates.json` | 即兴日常事件模板（6 种类型），编剧 Agent 消费 | 当前权威 |
| `story_outline.json` | 剧情大纲结构化数据（v3.0, 72 beats, 8 arcs），后端运行时读取 | v3.0 权威 |
| `剧情大纲.md` | 叙事圣经 v2.0（已被 `story/story_outline_full.md` v3.0 取代） | 参考 |
...
事件数据存在三层来源，修改时必须遵循以下层级：

| 层级 | 文件 | 说明 | 编辑规则 |
|---|---|---|---|
| **策划权威** | `事件配置表.csv` | 中文，55 行，人工维护 | ✅ 唯一可手动编辑的事件源 |
| **导出权威** | `events_complete.csv` | 英文，195 行，含 AI 展开子事件 | ❌ 勿手动编辑，由工具从主表导出 |
| **运行时 Mock** | `godot/data/mock/events.json` | 前端 Mock 子集，11 条 | ❌ 勿手动编辑，由工具从主表导出 |

> **注意**：如需添加/修改事件，只改 `事件配置表.csv`，然后重新运行导出工具生成英文版和 Mock 版。
